...
mu1 = c(0.45, 0.25, 0.5, 0.35, 0.3, 0.45, 0.45, 0.35, 0.35)
mu2 = c(0.3, 0.9, 0.3, 0.2, 0.1, 0.15, 0.2, 0.8, 0.3)
mu3 = c(0.4, 0.4, 0.4, 0.5, 0.5, 0.5, 0.4, 0.4, 0.4)
mu4 = c(0.5, 0.35, 0.2, 0.25, 0.35, 0.95, 0.75, 0.2, 0.25)
...
mu.all[,1:1500] <- mu[,1]
mu.all[,1501:3000] <- mu[,2]
mu.all[,3001:4000] <- mu[,3]
mu.all[,4001:5000] <- mu[,4]
mu.all[,5001:6500] <- mu[,5]
mu.all[,5001:6500] <- mu[,6]
mu.all[,6501:7500] <- mu[,7]
mu.all[,7501:9000] <- mu[,8]
mu.all[,9001:10000] <- mu[,9]
# best arm means vector 
mu.best <- apply(mu.all, 2, max)

# plot the mu.all
plot(1:T.times, mu.all[1,], type='l', ylim=c(0.1,1), xlab='Time', 
     ylab='Expected Reward of Each Arm', lty=1, axes=F)
axis(side=1)
axis(side=2, at=0.1*c(1:10)); box()
abline(v=c(1500,3000,4000,5000,6500,7500,9000), col = 'grey')
for(i in 1:K) lines(1:T.times, mu.all[i,], type='l',col=i, lty=i, lwd=2)
legend('topleft',c('Arm 1','Arm 2','Arm 3','Arm 4'),lwd=c(2,2,2,2),lty = 1:4, col=1:4, cex=0.8)
...
for(ii in 1:R){
  ...
  for(t in 1:T.times) reward.matrix[,t] <- rbinom(K,1,mu.all[,t])
  
  # add noise
  noise.index <- sample(1:T.times, 0.025*T.times)
  #noise.index = c(as.matrix(read.table('noise.index.txt')))
  reward.matrix[,noise.index] <- rbinom(0.025*T.times*K,1,0.99)
